 ...
 */
void
destroyMemory (memoryBlock * memory)
{
  memoryBlock *block;

  for (block = memory, memory = memory->next; memory;
       block = memory, memory = memory->next)
    {
      if (block->mem)
	free (block->mem);
      free (block);
 ...
 */
void
reallocMemory (memoryBlock * memory, size_t totalSpace, size_t requested)
{
  unsigned i;
  memoryBlock *block, *prev;
  size_t delta;			//Difference between total space and requested space

  if (requested > totalSpace)
    {				//Allocation Mechanism
      delta = requested - totalSpace;

      if (delta > 1024 * 1024)
	{			//Allocates in spans of 1MB each
      ...
      for (block = memory, prev = NULL; block;)
	{
	  ...
	  if (block->mem)
	    {
	      if (block->sz <= delta)
		{		//free more little blocks
		  delta -= block->sz;
		  free (block->mem);
		  if (prev)
		    {		//is this the frist block?
 ...
 */
void
printGraph (size_t maxMemory, float minY, float maxY, float minX, float maxX,
	    float step, unsigned timeStep, function f)
{
  float x, y;
...
main (int argc, char *argv[])
{
  ...
  printGraph ((size_t) (availableMemory * 0.9f), 0.0f, 5.0f, -3.0f, 3.0f,
	      0.001f, 3, dickFunction);
